...
<AccordionGroup>
  <Accordion title="Send and history semantics">
    ...
    - When Gateway session usage reports include context tokens, the chat composer area shows a compact context usage indicator. It switches to warning styling at high context pressure and, at recommended compaction levels, shows a compact button that runs the normal session compaction path. If the latest live update marks an existing count stale, the indicator stays visible with a `~` prefix until the Gateway reports fresh usage again; unknown token snapshots remain hidden.
